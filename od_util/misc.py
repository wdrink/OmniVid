...
def collate_fn(batch):
    ...
    for data in batch:
        video_list.append(data["video"])
        text_id_list.append(data["text"]["input_ids"])
        text_attn_list.append(data["text"]["attention_mask"])
        prompt_id_list.append(data["prompt"]["input_ids"])
        prompt_attn_list.append(data["prompt"]["attention_mask"])
        image_id_list.append(data["image_id"])
        class_id_list.append(data["class_id"])
        boxes_list.append(data["boxes"])
        orig_size_list.append(data["orig_size"])
        size_list.append(data["size"])
        caption_list.append(data["caption"])

    video_list = nested_tensor_from_tensor_list(video_list)
    text_list = {
    ...
    output = {
        ...
        "text": text_list,
        "prompt": prompt_list,
        "image_id": torch.stack(image_id_list, dim=0),
        "class_id": class_id_list,
        "boxes": boxes_list,
        "orig_size": torch.stack(orig_size_list, dim=0),
...
def nested_tensor_from_tensor_list_with_size_divisibility(

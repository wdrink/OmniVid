...
class AlproRetrieval(AlproBase):
    ...
    def forward(self, samples):
        ...
        video_feat = F.normalize(
            self.vision_proj(video_embeds[:, 0, :]), dim=-1
        )
        ...
        loss_v2t = -torch.sum(
            F.log_softmax(sim_v2t, dim=1) * sim_targets, dim=1
        ).mean()
        loss_t2v = -torch.sum(
            F.log_softmax(sim_t2v, dim=1) * sim_targets, dim=1
        ).mean()
    ...
    def compute_vtm(
        ...
        embedding_output_all = torch.cat(
            [text_embeds_all, video_embeds_all], dim=1
        )
    ...
    def compute_sim_matrix(self, data_loader, task_cfg):
        ...
        for i, sims in enumerate(
            ...
            attention_mask = torch.cat(
                [text_atts[topk_idx], video_atts_repeat], dim=1
            )
            embedding_output = torch.cat(
                [text_feats[topk_idx].to(self.device), video_feats_repeat],
                dim=1,
            )
        ...
        for i, sims in enumerate(
            ...
            text_atts_repeat = (
                text_atts[start + i].repeat(k_test, 1).to(self.device)
            )
            ...
            embedding_output = torch.cat(
                [text_feats_repeat, video_feats[topk_idx].to(self.device)],
                dim=1,
            )
    ...
    def from_config(cls, cfg):
        ...
        num_patches = (
            visual_encoder_config["image_size"]
            // visual_encoder_config["patch_size"]
        ) ** 2

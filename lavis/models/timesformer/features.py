    ...
    def get_dicts(self, keys=None, idx=None):
        ...
        if idx is None:
            if keys is None:
                ...
                return [
                    {k: self.info[i][k] for k in keys} for i in self.out_indices
                ]
        if isinstance(idx, (tuple, list)):
            return [
                self.info[i]
                if keys is None
                else {k: self.info[i][k] for k in keys}
                for i in idx
            ]
        else:
            return (
                self.info[idx]
                if keys is None
                else {k: self.info[idx][k] for k in keys}
            )
...
class FeatureHooks:
    ...
    def __init__(
        self, hooks, named_modules, out_map=None, default_hook_type="forward"
    ):
        # setup feature hooks
        ...
        for i, h in enumerate(hooks):
            ...
            hook_type = (
                h["hook_type"] if "hook_type" in h else default_hook_type
            )
...
class FeatureHookNet(nn.ModuleDict):
    ...
    def __init__(
        ...
        if no_rewrite:
            ...
            ):  # make sure classifier is removed?
            ...
            modules = _module_list(model, flatten_sequential=flatten_sequential)
            remaining = {
                f["module"]: f["hook_type"]
                if "hook_type" in f
                else default_hook_type

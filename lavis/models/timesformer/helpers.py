...
def load_checkpoint(
    ...
    if init_2d_patch_embed and "patch_embed.proj.weight" in state_dict:
        # cp the weights of 3d patch embed to 2d patch embed
        threed_proj = state_dict[
            "patch_embed.proj.weight"
        ]  # shape: (embed_dim, 3, 2, patch_size, patch_size)
        threed_proj_bias = state_dict[
            "patch_embed.proj.bias"
        ]  # shape: (embed_dim,)
        threed_norm = state_dict[
            "patch_embed.norm.weight"
        ]  # shape: (embed_dim,)
        threed_norm_bias = state_dict[
            "patch_embed.norm.bias"
        ]  # shape: (embed_dim,)

        # opt1: use the mean of the 3d patch embed weights
        twod_proj = threed_proj.mean(
            dim=2
        )  # shape: (embed_dim, patch_size, patch_size)
        # opt2: use the first 2d patch embed weights
        # twod_proj = threed_proj[:, :, 0, :, :]

        twod_proj_bias = threed_proj_bias
        twod_norm = threed_norm
        twod_norm_bias = threed_norm_bias

        state_dict["patch_embed2d.proj.weight"] = twod_proj
        state_dict["patch_embed2d.proj.bias"] = twod_proj_bias
        state_dict["patch_embed2d.norm.weight"] = twod_norm
        state_dict["patch_embed2d.norm.bias"] = twod_norm_bias

    mgs = model.load_state_dict(state_dict, strict=strict)
    print("Missing keys:", mgs.missing_keys)
    print("Unexpected keys:", mgs.unexpected_keys)
...
def load_pretrained_kinetics(
    ...
    **kwargs,
    ...
    ## Resizing the positional embeddings in case they don't match
    if num_patches + 1 != state_dict["pos_embed"].size(1):
        new_pos_embed = resize_spatial_embedding(
            state_dict, "pos_embed", num_patches
        )
        state_dict["pos_embed"] = new_pos_embed

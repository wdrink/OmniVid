...
class AlbefVQA(AlbefBase, MomentumDistilationMixin):
    ...
    def _rampup_factor(self, epoch, iters, num_iters_per_epoch):
        return min(
            1, (epoch * num_iters_per_epoch + iters) / num_iters_per_epoch
        )
    ...
    def predict_answers(
        self, samples, answer_list, num_ans_candidates=128, **kwargs
    ):
        """
        Args:
            samples (dict): A dictionary containing the following keys:
    ...
    def from_config(cls, cfg=None):
        ...
        config_decoder = BertConfig.from_json_file(
        ...
        for key in list(state_dict.keys()):
            ...
            if "text_encoder" in key:
                if "layer" in key:
                    ...
                    encoder_key = key
                decoder_key = encoder_key.replace(

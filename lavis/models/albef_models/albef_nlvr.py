...
class AlbefNLVR(AlbefBase, MomentumDistilationMixin):
    ...
    def _rampup_factor(self, epoch, iters, num_iters_per_epoch):
        return min(
            1, (epoch * num_iters_per_epoch + iters) / (2 * num_iters_per_epoch)
        )
    ...
    def forward(self, samples, is_train=True):
        ...
        image0_embeds, image1_embeds = torch.split(
            image_embeds, targets.size(0)
        )
            ...
            return AlbefOutput(
                ...
                intermediate_output=AlbefIntermediateOutput(
                    image_embeds=torch.stack(
                        [image0_embeds, image1_embeds], dim=0
                    ),
    ...
    def from_config(cls, cfg=None):
        ...
        assert (
            num_classes > 1
        ), "Invalid number of classes provided, found {}".format(num_classes)

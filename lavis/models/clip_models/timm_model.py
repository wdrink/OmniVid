...
def pixel_freq_bands(
    ...
    if linear_bands:
        bands = torch.linspace(
            1.0, max_freq / 2, num_bands, dtype=dtype, device=device
        )
    else:
...
def build_fourier_pos_embed(
    ...
    if bands is None:
        if in_pixels:
            ...
            bands = inv_freq_bands(
                num_bands, step=1, dtype=dtype, device=device
            )
    else:
        if device is None:
            device = bands.device
    ...
    if in_pixels:
        grid = torch.stack(
            torch.meshgrid(
                [
                    torch.linspace(
                        -1.0, 1.0, steps=s, device=device, dtype=dtype
                    )
                    for s in feat_shape
                ]
            ),
...
class FourierEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.keep_spatial:
            x = torch.cat(
                [x, emb.unsqueeze(0).expand(batch_expand).permute(0, 3, 1, 2)],
                dim=1,
            )
        else:
            x = torch.cat(
                [x.permute(0, 2, 3, 1), emb.unsqueeze(0).expand(batch_expand)],
                dim=-1,
            )

...
class VisionTransformer(nn.Module):
    ...
    def online_interpolate_pos(self, expected_shape):
        embedding_size = self.embed_dim
        # num_patches = expected_len

        num_extra_tokens = (
            self.pos_embed.shape[1] - self.patch_embed.num_patches
        )
        # height (== width) for the checkpoint position embedding
        orig_size = int((self.pos_embed.shape[1] - num_extra_tokens) ** 0.5)
        # height (== width) for the new position embedding
        # new_size = expected_shape
        # class_token and dist_token are kept unchanged
        # print(
        #    "Position interpolate from %dx%d to %dx%d"
        #    % (orig_size, orig_size, expected_shape[0], expected_shape[1])
        # )
    ...
    def forward_features(self, x):
        expected_shape = (
            x.shape[2] // self.patch_embed.patch_size[0],
            x.shape[3] // self.patch_embed.patch_size[1],
        )
        ...
        if self.pos_embed is not None:
            if self.pos_embed.shape[1] == x.shape[1]:
                ...
                pos = self.online_interpolate_pos(expected_shape)

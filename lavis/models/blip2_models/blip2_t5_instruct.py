...
from lavis.models.blip2_models.modeling_t5 import (
    T5Config,
    ...
    def prepare_few_shot_embeds(self, samples):
        ...
        if this_n_fs > 1:
            encoder_atts = encoder_atts.reshape(
                encoder_atts.size(0) // this_n_fs,
                encoder_atts.size(1) * this_n_fs,
            )
    ...
    def _predict_class(
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            for n in range(n_segments):
                ...
                this_encoder_outputs[
                    "last_hidden_state"
                ] = this_encoder_outputs[0].repeat_interleave(seg_len, dim=0)
                this_encoder_atts = encoder_atts.repeat_interleave(
                    seg_len, dim=0
                )

                start_i = n * (n_cands // n_segments)
                end_i = start_i + seg_len
                this_output_tokens_ids = output_tokens.input_ids[
                    start_i:end_i
                ].repeat(bs, 1)
                ...
                this_targets = this_output_tokens_ids.masked_fill(
                    this_output_tokens_ids == self.t5_tokenizer.pad_token_id,
                    -100,
                )

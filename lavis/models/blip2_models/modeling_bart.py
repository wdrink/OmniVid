...
from od_util.box_loss import SIoU_loss
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, use_iou_loss, iou_loss_weight):
        super().__init__(config)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if target_token_weights is None:
                ...
                masked_lm_loss = loss_fct2(
                    lm_logits.view(-1, self.config.vocab_size), labels.view(-1)
                )
                # print(target_token_weights.shape, target_token_weights.sum())
                masked_lm_loss = (
                    masked_lm_loss * target_token_weights.view(-1)
                ).mean()

        # only during training, we need to compute the loss
        if self.use_iou_loss and labels is not None:
            assert gt_boxes is not None and aug_shapes is not None
            # gt_boxes: bs x 4 (normalized)
            # aug_shapes: bs x 2 x 2
            aug_heights = aug_shapes[:, 0, 0]
            aug_widths = aug_shapes[:, 0, 1]
            gt_boxes[:, 0] = gt_boxes[:, 0] * aug_widths
            gt_boxes[:, 1] = gt_boxes[:, 1] * aug_heights
            gt_boxes[:, 2] = gt_boxes[:, 2] * aug_widths
            gt_boxes[:, 3] = gt_boxes[:, 3] * aug_heights

            pretrained_tokenizer_length = tokenizer.pretrained_tokenizer_length
            box_x_values = []
            box_y_values = []
            num_boxes = len(tokenizer.box_tokens)
            for i in range(num_boxes):
                box_x_value = i * aug_widths / (tokenizer.num_bins - 1)
                box_y_value = i * aug_heights / (tokenizer.num_bins - 1)
                box_x_values.append(box_x_value)
                box_y_values.append(box_y_value)

            box_x_values = torch.stack(box_x_values, dim=1)
            box_y_values = torch.stack(box_y_values, dim=1)

            lm_logits_after_softmax = torch.softmax(lm_logits, dim=2)[
                :,
                :,
                pretrained_tokenizer_length : pretrained_tokenizer_length
                + num_boxes,
            ]

            x1_expections = (lm_logits_after_softmax[:, 2] * box_x_values).sum(
                dim=1
            )
            y1_expections = (lm_logits_after_softmax[:, 3] * box_y_values).sum(
                dim=1
            )
            x2_expections = (lm_logits_after_softmax[:, 4] * box_x_values).sum(
                dim=1
            )
            y2_expections = (lm_logits_after_softmax[:, 5] * box_y_values).sum(
                dim=1
            )

            box_expections = torch.stack(
                [x1_expections, y1_expections, x2_expections, y2_expections],
                dim=1,
            )
            # print(box_expections, gt_boxes)
            siou_loss, _ = SIoU_loss(box_expections, gt_boxes)
            siou_loss = siou_loss.mean()
            # print("SIoU: ", siou_loss, "LM: ", masked_lm_loss)
            masked_lm_loss += self.iou_loss_weight * siou_loss

...
class VideoLLAMA(Blip2Base):
    ...
    def __init__(
        ...
        if llama_proj_model:
            ...
            msg = model.load_state_dict(
                llama_proj_weight["model"], strict=False
            )
    ...
    def encode_img(self, image):
        ...
        with self.maybe_autocast():
            ...
            atts_llama = torch.ones(
                inputs_llama.size()[:-1], dtype=torch.long
    ...
    def forward(self, samples):
        if "conv_type" in samples.keys() and samples["conv_type"] == "multi":
            ...
            temp_input_embedding = self.llama_model.model.embed_tokens(
                temp_input_ids
            )
            ...
            for cur_input_ids, cur_input_embeds in zip(
                input_ids, temp_input_embedding
            ):
                cur_image_features = img_embeds[cur_image_idx]

                if (
                    cur_input_ids == im_patch_token_id
                ).sum() != num_patch_tokens:
                    raise ValueError(

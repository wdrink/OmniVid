        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            for n in range(n_segments):
                ...
                this_input_tokens_ids = (
                    text_input_tokens.input_ids.repeat_interleave(
                        seg_len, dim=0
                    )
                )
                this_input_tokens_atts = (
                    text_input_tokens.attention_mask.repeat_interleave(
                        seg_len, dim=0
                    )
                )
                ...
                this_output_tokens_atts = (
                    this_output_tokens.attention_mask.repeat(bs, 1)
                )

                (
                    this_llm_tokens,
                    this_input_targets_len,
                ) = self.concat_text_input_output(
                    this_input_tokens_ids,
                    this_input_tokens_atts,
                ...
                inputs_embeds = torch.cat(
                    [
                        inputs_llm.repeat_interleave(seg_len, dim=0),
                        inputs_embeds,
                    ],
                    dim=1,
                )

...
from lavis.models.timesformer.helpers import load_checkpoint
...
class PatchEmbed2D(nn.Module):
    def __init__(
        self, patch_size=(4, 4), in_chans=3, embed_dim=96, norm_layer=None
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # BD C Wh Ww
        if self.norm is not None:
            Wh, Ww = x.size(2), x.size(3)
            x = x.flatten(2).transpose(1, 2)
            x = self.norm(x)
            # BD, WhWw, C -> BD, C, WhWh -> B, D, C, Wh, Ww - > B, C, D, Wh, Ww
            x = (
                x.transpose(1, 2)
                .view(-1, D, self.embed_dim, Wh, Ww)
                .transpose(1, 2)
            )
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(
        ...
        self.window_size = window_size
        self.patch_size = patch_size
        self.init_3d_patch_embed = init_3d_patch_embed
        self.init_2d_patch_embed = init_2d_patch_embed

        if self.init_3d_patch_embed:
        ...
        if isinstance(self.pretrained, str):
            ...
            if self.pretrained2d:
                ...
                load_checkpoint(
                    ...
                    init_3d_patch_embed=self.init_3d_patch_embed,
                    init_2d_patch_embed=self.init_2d_patch_embed,
                )
    ...
    def forward(self, x, return_fpn=False):
        ...
        return fpn if return_fpn else fpn[-1]
...
def create_videoswin(
    ...
    if pretrained == "k400":
        url = "https://github.com/SwinTransformer/storage/releases/download/v1.0.4/swin_base_patch244_window877_kinetics400_22k.pth"
        pretrained2d = False
    elif pretrained == "k600":
        url = "https://github.com/SwinTransformer/storage/releases/download/v1.0.4/swin_base_patch244_window877_kinetics600_22k.pth"
        pretrained2d = False
    elif pretrained == "in21k":

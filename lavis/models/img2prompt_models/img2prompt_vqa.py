...
class Img2PromptVQA(BaseModel):
    ...
    def itm_rank(
        self, image_embeds, image_atts, encoder_input_ids, match_head="itm"
    ):
        # breakpoint()
        encoder_input_ids = encoder_input_ids.clone()
        encoder_input_ids = encoder_input_ids[:, self.prompt_length - 1 :]
        text_attention_mask = (
            encoder_input_ids != self.tokenizer.pad_token_id
        ).long()
    ...
    def forward_cap(
        ...
        while min_num_captions < num_captions:
            ...
            prompt.input_ids[
                :, 0
            ] = self.image_captioning_model.tokenizer.bos_token_id
            ...
            for counter, output in enumerate(outputs):
                ...
                if len(captions[ind]) < num_captions:
                    ...
                    overlap_caption = [
                        1 for caps in captions[ind] if caption in caps
                    ]
                    # print(itm_outputs)
                    if (
                        len(overlap_caption) == 0
                        and itm_outputs[counter] >= 0.5
                    ):  # image filter
    ...
    def create_context_prompt(self, samples, num_caps_per_img=30):
        ...
        for idx in range(num_caps_per_img):
            cap_id_list = ans_dict_queid.get(
                answers[(len(answers) - 1 - idx) % len(answers)][:-1].lower(),
                [0],
            )
    ...
    def create_task_prompt(
        ...
        for idx in range(num_question_per_img):
            ...
            if "question_type" == "rule":  # Rule-Based Question Generation
                ...
                Task_Prompt += "Answer:"
                Task_Prompt += syn_ans_queid[(qa_idx) % len(syn_ans_queid)][
                    :-1
                ].lower()
                Task_Prompt += "\n"
    ...
    def prepare_LLM_input(
        ...
        if self.offload_model:
            self.image_question_matching_model.to(
                self.question_answering_model.device
            )

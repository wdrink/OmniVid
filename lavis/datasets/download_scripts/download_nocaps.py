...
# Setup
logging.basicConfig(
    filename="download_nocaps.log", filemode="w", level=logging.INFO
)
...
if __name__ == "__main__":
    ...
    storage_dir = OmegaConf.load(
        config_path
    ).datasets.nocaps.build_info.images.storage
    ...
    with Pool(n_procs) as pool:
        ...
        list(
            tqdm.tqdm(
                pool.imap(download_image_from_url_val, val_urls),
                total=len(val_urls),
            )
        )
    ...
    with Pool(n_procs) as pool:
        ...
        list(
            tqdm.tqdm(
                pool.imap(download_image_from_url_test, tst_urls),
                total=len(tst_urls),
            )
        )

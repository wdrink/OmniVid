...
class VQATask(BaseTask):
    ...
    def after_evaluation(self, val_result, split_name, **kwargs):
        ...
        metrics = self._report_metrics(
            result_file=result_file, split=split_name
        )
    ...
    def _report_metrics(self, result_file, split):
        ...
        if split in self.ques_files and split in self.anno_files:
            ...
            with open(
                os.path.join(registry.get_path("output_dir"), "evaluate.txt"),
                "a",
            ) as f:
...
class AOKVQATask(VQATask):
    ...
    def _save_result_leaderboard(self, results):
        ...
        logging.info(
            f"Saved results for leaderboard evaluation at {result_file}"
        )

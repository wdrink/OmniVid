...
class ANETcaptions(object):
    ...
    def __init__(
        ...
        self.ground_truths_keys = [
            vid for gt in self.ground_truths for vid in gt
        ]
        print(
        ...
        print(
            "gt video number",
            len(set(self.ground_truths_keys)),
        )
    ...
    def import_prediction(self, prediction_filename):
        ...
        for vid_id in submission["results"]:
            if not vid_id.startswith("v_"):
                real_vid_id = "v_" + vid_id
            else:
                real_vid_id = vid_id
            
            results[real_vid_id] = submission["results"][vid_id][
                : self.max_proposals
            ]
        return results
            ...
            for gt in self.ground_truths:
                ...
                if vid_id in self.prediction:
                    for pred_i, pred in enumerate(self.prediction[vid_id]):
                        ...
                        for ref_i, ref_timestamp in enumerate(
                            refs["timestamps"]
                        ):
                ...
                new_recall = float(len(ref_set_covered)) / len(
                    refs["timestamps"]
                )
    ...
    def evaluate_tiou(self, tiou):
                ...
                for pred in self.prediction[vid_id]:
                    ...
                    for gt in self.ground_truths:
                        ...
                        for caption_idx, caption_timestamp in enumerate(
                            ...
                            if (
                                self.iou(pred["timestamp"], caption_timestamp)
                                >= tiou
                            ):
        ...
        for scorer, method in self.scorers:
            ...
            for vid in vid2capid.keys():
                res[vid] = {
                    index: tokenize_res[index] for index in vid2capid[vid]
                }
                gts[vid] = {
                    index: tokenize_gts[index] for index in vid2capid[vid]
                }
...
def main(args):
    ...
    evaluator = ANETcaptions(
        ...
        no_lang_eval=args.no_lang_eval,
    )
    evaluator.evaluate()
...
if __name__ == "__main__":
    ...
    avg_eval_score = {
        key: np.array(value).mean() for key, value in score.items()
    }

...
class SODA:
    def __init__(
        self, data, soda_type="c", tious=None, scorer="Meteor", verbose=False
    ):
        # self.data = data
    ...
    def calc_iou_matrix(self, preds, golds):
        ...
        return np.array(
            [
                [iou(pred, ct) for pred in preds["timestamps"]]
    ...
    def calc_score_matrix(self, preds, golds):
        ...
        gts = [
            {index: g for index in range(len(p_sent))}
            for i, g in enumerate(g_sent)
        ]
    ...
    def evaluate(
        ...
        if self.verbose:
            for i, tiou in enumerate(tious):
                partial_result = {
                    self.scorer_name: [precision[i], recall[i], f1[i]]
                }
...
def main(args):
    # Call coco eval
    data = ANETCaptions.from_load_files(
        args.references,
        args.prediction,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="Meteor",
        choices=["Meteor", "Cider", "BertScore"],
    ...
    parser.add_argument(
        "-mr", "--multi_reference", action="store_true", help="print details"
    )
    parser.add_argument(

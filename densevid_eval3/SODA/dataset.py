...
class ANETCaptions:
    ...
    @classmethod
    def from_load_files(
        cls, gt_file, pred_file, multi_reference=True, verbose=False
    ):
        gts, gt_vid = cls.load_ground_truth(
            gt_file, multi_reference=multi_reference
        )
        preds = cls.load_prediction(pred_file)
        gt_vid = cls.check_videos(gt_vid, preds.keys(), verbose=verbose)
        return cls(preds, gts, gt_vid, verbose=verbose)

    @classmethod
    def from_prediction(
        cls, gt_file, preds, multi_reference=True, verbose=False
    ):
        results = {}
        for vid in preds["results"]:
            results[vid] = sorted(
                preds["results"][vid], key=lambda x: x["timestamp"][0]
            )
        gts, gt_vid = cls.load_ground_truth(
            gt_file, multi_reference=multi_reference
        )
        ...
        if multi_reference is False:
            for vid in gt_vids:
                ...
                for _g in gts:
                    ...
                    s += _g[vid]["sentences"]
                sort_t, sort_s = list(
                    zip(*sorted(zip(t, s), key=lambda x: x[0][0]))
                )
    ...
    def load_prediction(filename, verbose=False):
        ...
        for vid in pred["results"]:
            ...
            results[vid] = sorted(
                pred["results"][vid], key=lambda x: x["timestamp"][0]
            )
        return results
            ...
            cur_preds.update(
                {
                    j
                    + p_spliter[-1]: [
                        {"caption": remove_nonascii(p["sentence"])}
                    ]
                    for j, p in enumerate(self.preds[vid])
                }
            )
                ...
                cur_gts[n].update(
                    {
                        ...
                        for j, p in enumerate(self.gts[n][vid]["sentences"])
                    }
                )
                g_spliter[n].append(
                    g_spliter[n][-1] + len(self.gts[n][vid]["sentences"])
                )
        tokenize_preds = self.tokenizer.tokenize(cur_preds)
        tokenize_gts = [self.tokenizer.tokenize(j) for j in cur_gts]
        for i, vid in enumerate(self.gt_vids):
            _p = [
                tokenize_preds[j] for j in range(p_spliter[i], p_spliter[i + 1])
            ]
    ...
    def check_videos(gold_vid, pred_vid, verbose=True):
        ...
        if len(not_appear) > 0 and verbose:
            print(
                (
                    f"Warning: some videos in ground truth file are not appeared in prediction file!\n"

...
def eval_tool(
    prediction, referneces=None, metric="Meteor", soda_type="c", verbose=False
):
...
def eval_soda(p, ref_list, verbose=False):
    ...
    for ref in ref_list:
        r = eval_tool(
            prediction=p, referneces=[ref], verbose=verbose, soda_type="c"
        )
...
if __name__ == "__main__":
    ...
    for p in [p_new, p_vitr]:
        ...
        print(p)
        ref_list = [
            "data/anet/captiondata/val_1.json",
            "data/anet/captiondata/val_2.json",
        ]
        score = eval_soda(p, ref_list, verbose=False)
        print(score)
        para_score = get_para_score(
            p,
            referneces=[
